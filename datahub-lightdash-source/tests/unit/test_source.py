...
from datahub_lightdash_source.config import LightdashSourceConfig  # noqa: E402
...
from datahub_lightdash_source.source import LightdashSource  # noqa: E402
...
def source():
    ...
    src.client.get_project.return_value = LightdashProject(
        ...
        name="Webstore",
        warehouseConnection=LightdashWarehouseConnection(type="clickhouse", schema="default"),
    )
...
def test_chart_emits_input_fields_for_column_lineage(source):
    """A chart referencing dimensions ``orders_enriched_order_id`` /
    ``orders_enriched_order_status`` should produce one ``inputFields`` entry
    per referenced warehouse column, pointing at the right schemaFieldUrn on
    the upstream ClickHouse dataset.
    """
    # Wire up the chart with explicit dimensions + metrics.
    ...
    # And the explore exposes field metadata for those.
    ...
    # Dimensions resolve directly; the metric ``total_revenue`` aggregates the
    # column ``line_total_eur`` extracted from its ``${TABLE}.line_total_eur`` SQL.
    ...
    # The embedded SchemaField must carry a fieldPath — otherwise DataHub's
    # pipeline drops the aspect at validation time.
